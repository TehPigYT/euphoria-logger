...
class Logger {
  ...
  log(type, message, show_memory = false) {
    if (
      !this.sources.webhooks.length &&
      !this.sources.files.length &&
      !this.sources.console
    ) {
      console.error(
        `[LOGGER] You must either enable console logs, file logs, or connect a webhook.`
      );
    ...
    const finalType =
      this.typeFormats[type.toUpperCase()] || type.toUpperCase();
    const memoryUsage = show_memory
      ? measureSize(process.memoryUsage().rss)
      : "";
    ...
    // Log to console
    if (this.console) {
      logToConsole(this, this.format(logMessage), type);
    }

    // Log to webhooks
    if (this.sources.webhooks.length) {
      logToWebhooks(this, logMessage, type);
    }
    ...
    // Log to files
    if (!this.bufferInterval) {
      logToFiles(this, logMessage, type);
    }
  }
  ...
  info(message, show_memory = false) {
    if (!this.allowedTypes.has("INFO")) {
      console.log(
        "[LOGGER] The info type does not exist in the provided types."
      );
      return;
    }
  ...
  success(message, show_memory = false) {
    if (!this.allowedTypes.has("SUCCESS")) {
      console.log(
        "[LOGGER] The success type does not exist in the provided types."
      );
      return;
    }
  ...
  warn(message, show_memory = false) {
    if (!this.allowedTypes.has("WARN")) {
      console.log(
        "[LOGGER] The warn type does not exist in the provided types."
      );
      return;
    }
  ...
  error(message, show_memory = false) {
    if (!this.allowedTypes.has("ERROR")) {
      console.log(
        "[LOGGER] The error type does not exist in the provided types."
      );
      return;
    }
  ...
  debug(message, show_memory = false) {
    if (!this.allowedTypes.has("DEBUG")) {
      console.log(
        "[LOGGER] The debug type does not exist in the provided types."
      );
      return;
    }
